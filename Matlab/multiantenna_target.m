...
% K = number of base station
% M = number of antennas per mobile
% L = number of antennas per base station
...
for l=1:length(l_actual)
    ...
    for j=1:reps
        ...
        for k=1:K
            h1(1+l_actual(l)*(k-1):l_actual(l)*k,:)=h1(1+l_actual(l)*(k-1):l_actual(l)*k,:).*antenna_r(k).^(-alpha/2);
        end  
